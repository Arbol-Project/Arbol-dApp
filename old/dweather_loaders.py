...
from risk.indices.loaders.base import DataLoader
from risk.utils import box
from risk.tools.serialize import deserialize_func, serialize_func

API_BASE_URL = 'https://api.dclimate.net/apiv2'
...
def pull_data(url, params):
    ...
    try:
        ...
        if len(result) == 1 and '404' in result:
            raise ValueError('{}, request: {}'.format(result['404'], url))
        elif len(result) == 2 and 'errors' in result:
            raise ValueError(str(result))
    except JSONDecodeError:
        raise ValueError('API response: {}'.format(api_response.content.decode()[:200]))
...
class DWeatherLoader(DClimateLoader):

    """
    This class is deprecated. The DClimateLoader should be used instead
    """
    def __init__(self, *args, **kwargs):
        warnings.warn('DWeatherLoader class is deprecated. The DClimateLoader should be used instead')
...
class GridcellLoader(DClimateLoader):
    ...
    def _api_load(self):
        request = '{}/grid-history/{}/{}_{}'.format(
            API_BASE_URL, self._dataset_name, self._location['lat'], self._location['lon']
        )
